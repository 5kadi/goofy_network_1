...
class Optim:
    ...
    def calculate_derivative(self, layer: BaseLayer, next_layer_error: Matrix) -> Matrix:
        ...
        weights: Matrix = layer.weights.matrix
        signal: Matrix = layer.signal.matrix
        error: Matrix = next_layer_error.matrix
        for k in range(layer.outputs): 
            ...
            for j in range(layer.inputs):
                new_derivative: float = -error[k][0]*new_sigmoid*(1 - new_sigmoid)*signal[j][0]
